...
function initClient() {
    if (!DetectRTC.isMobileDevice) {
        ...
        setTippy('volumnControl', 'Volume control', 'right');
        ...
        setTippy('startRecButton2', 'Start Recording', 'right');
...
function whoAreYou() {
    ...
    Swal.fire({
        ...
        inputValidator: (name) => {
            ...
            setCookie(room_id + '_naroom_idme', name, 30);
            peer_name = name;
        },
    }).then(() => {
        if (initStream) {
...
function handleJoinMeeting () {
    peer_name = document.getElementById('nick-name').value
    if (!peer_name) alert('Please enter your name');
    else {
        if (!getCookie(room_id + '_name')) {
        window.localStorage.peer_name = peer_name;
    }
    setCookie(room_id + '_naroom_idme', peer_name, 30);
    if (initStream) {
        stopTracks(initStream);
        hide(initVideo);
    }
    getPeerInfo();
    joinRoom(peer_name, room_id);
    document.getElementById("initUser").style.display = "none"
    // Swal.close()
...
function handleButtons() {
    ...
    volumnControl.onclick = () => {
        document.getElementById("vol-control").style.display = 'block'
    }
    settingsButton.onclick = () => {
        rc.toggleMySettings();
    };
    startRecButton2.onclick = () => {
        document.getElementById('stopRecButton2').style.display = 'block'
        document.getElementById('startRecButton2').style.display = 'none'
        rc.startRecording();
    }
    stopRecButton2.onclick = () => {
        document.getElementById('stopRecButton2').style.display = 'none'
        document.getElementById('startRecButton2').style.display = 'block'
        rc.stopRecording()
    }
...
function SetVolume(val)
{
    ...
    var player = document.querySelector("video[name='"+ peer_info.peer_id + "']")
    console.log(player)
